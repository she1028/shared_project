...
<head>
    ...
    <style>
        /* Chatbot button */
        #chatBotLink { background-color: #8c580a; cursor: pointer; }
        .chatbot-text { opacity: 0; max-width: 0; overflow: hidden; white-space: nowrap; transform: translateX(-5px); transition: opacity 0.5s ease, transform 0.5s ease, max-width 0.5s ease; }
        #chatBotLink:hover .chatbot-text { opacity: 1; max-width: 200px; transform: translateX(0); }

        /* Chat Modal */
        #chatModal { display: none; position: fixed; z-index: 1100; left: 10px; bottom: 70px; border-radius: 8px; box-shadow: 0 0 15px rgba(0,0,0,0.1); overflow: hidden; transition: all 0.3s ease-in-out; background-color:#fff; width:300px; height:400px; }
        #chatModalContent { height:100%; display:flex; flex-direction:column; padding:15px; position:relative; }
        #chatModalClose { position:absolute; top:5px; right:10px; font-size:24px; cursor:pointer; color:#999; }
        #chat { flex-grow:1; overflow-y:auto; padding-bottom:10px; font-family:Arial, sans-serif; }
        #chat p { margin:5px 0; padding:8px 12px; border-radius:8px; line-height:1.4; }
        #chat p.user { background:#d4efff; align-self:flex-end; width:fit-content; }
        #chat p.bot { background:#f1f1f1; align-self:flex-start; width:fit-content; }

        /* Input container */
        #input-container { display:flex; gap:10px; margin-top:10px; }
        #input { flex:1; padding:10px 15px; border-radius:8px; border:1px solid #000; font-size:16px; }
    </style>
</head>
<body>
